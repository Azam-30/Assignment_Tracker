document.getElementById('loadData').addEventListener('click', function () {
    ...
    .then(data => {
        ...
        let pythonCount = data.python_count;
        ...
        let pythonCountElement = document.getElementById('python-count');
        if (pythonCountElement) {
            pythonCountElement.innerHTML = `Number of assignments submitted for Python: <span class="count-highlight">${pythonCount}</span>`;
        }

        // Data Structures Table
        let dataStructuresData = data.data_structures_data;
        let dataStructuresCount = data.data_structures_count;
        ...
        let dataStructuresCountElement = document.getElementById('data-structures-count');
        if (dataStructuresCountElement) {
            dataStructuresCountElement.innerHTML = `Number of assignments submitted for Data Structures: <span class="count-highlight">${dataStructuresCount}</span>`;
        }

        if (pythonData.length > 0 || dataStructuresData.length > 0) {
            messageDiv.textContent = 'Data loaded successfully!';
            messageDiv.style.color = 'green';
            document.getElementById('tables').style.display = 'block';  // Show the tables
        } else {
            messageDiv.textContent = 'No data found for the provided roll number.';
            messageDiv.style.color = 'red';
            document.getElementById('tables').style.display = 'none';  // Hide the tables if no data
        }
    })
    .catch(error => {
